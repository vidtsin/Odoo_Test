...
class TdsWizard(models.TransientModel):
    ...
    def action_print_timesheet_analysis(self):
        ...
        worksheet.write(6, 3, _('VENDOR REF.'), column_heading_style)
        worksheet.write(6, 4, _('PARTY CODE'), column_heading_style)
        worksheet.write(6, 5, _('VENDOR NAME'), column_heading_style)
        worksheet.write(6, 6, _('DEDUCTEE PAN NO.'), column_heading_style)
        worksheet.write(6, 7, _('ASSESSEE CODE'), column_heading_style)
        worksheet.write(6, 8, _('TDS SEC'), column_heading_style)
        worksheet.write(6, 9, _('TDS %'), column_heading_style)
        worksheet.write(6, 10, _('TDS BASE AMOUNT'), column_heading_style)
        worksheet.write(6, 11, _('TDS AMOUNT'), column_heading_style)
        ...
        for wizard in self:
            ...
            total = 0
            count  = 0
            for tds_name in self.env['account.invoice'].search([('type','=','in_invoice'),('state','=','open'),('create_date','>=',wizard.date_start),('create_date','<=',wizard.date_end)]):
                        ...
                        for line_3 in self.env['account.invoice.line'].search([('invoice_id','=',line_1.id),]):
                            for line_5 in line_3.tds_nod_id.name:
                                for line_4 in self.env['account.tds.mapping'].search([('name','=',line_5.name)]):
                                    ...
                                    worksheet.write(row, 1, line_1.date_invoice, easyxf('font:height 200; align: horiz left;'))
                                    worksheet.write(row, 2, line_1.number, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 3, line_1.reference, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 4, line_1.partner_id.ref, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 5, line_1.partner_id.name, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 6, line_1.partner_id.pan_no, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 7, line_2.assesse_code.name, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 8, line_4.tds_group.tds_section.name, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 9, line_4.tds, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 10, line_1.amount_untaxed, easyxf('font:height 200;align: horiz left;'))
                                    worksheet.write(row, 11, line_3.total_tds_amount, easyxf('font:height 200;align: horiz left;'))
                                    row += 1
                    ...
                    for line_5 in line_3.tds_nod_id.name:
                        ...
                        # for line_3 in tds_p:
                            docs.append({
                                    ...
                                    'tds_base':line_1.amount_untaxed,
